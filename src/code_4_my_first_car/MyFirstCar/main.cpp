...
int main(void)
{
    ...
    /*Car rotation and scale*/
    glm::mat4 carRot = glm::translate(glm::mat4(1.f),glm::vec3(0.0,0.0,-2.0));
    carRot = glm::rotate(carRot, 0.1f, glm::vec3(-1.0,0.0,0.0));
    carRot = glm::translate(carRot, glm::vec3(0.0,0.0,2.0));

    glm::mat4 carRes = glm::scale(carRot,glm::vec3(1.0,1.0,1.7));
    carRes = glm::translate(carRes,glm::vec3(0.0,0.0,0.0));

    /*Wheels*/
    glm::mat4 wheelResize = glm::mat4(1.f);
    wheelResize = glm::rotate(wheelResize,1.57f,glm::vec3(0.0,0.0,1.0));
    wheelResize = glm::scale(wheelResize,glm::vec3(0.5,0.1,0.5));

    //translate, scale, rotate, shear

    glm::mat4 wheels[4];
    wheels[0] = glm::translate(glm::mat4(1.f),glm::vec3(1.2,-0.6,1.2)) * wheelResize;

    wheels[1] = glm::translate(glm::mat4(1.f),glm::vec3(-1.0,-0.6,1.2)) * wheelResize;

    wheels[2] = glm::translate(glm::mat4(1.f),glm::vec3(1.2,-0.8,-1.2)) * wheelResize;

    wheels[3] = glm::translate(glm::mat4(1.f),glm::vec3(-1.0,-0.8,-1.2)) * wheelResize;
    ...
    glm::mat4 cube_to_spoiler;
        ...
        glm::mat4 a4 = glm::translate(glm::mat4(1.0), glm::vec3(0.0, 1.0, 0.15));
        cube_to_spoiler = carRot*a1*a2*a3*a4;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        */
        for(int i=0; i<4; i++){
        glUniformMatrix4fv(basic_shader["uT"],1,GL_FALSE,&wheels[i][0][0]);
        glUniform3f(basic_shader["uColor"],0.2,0.5,1.0);
        r_cyl.bind();
        glDrawElements(GL_TRIANGLES,r_cyl.in, GL_UNSIGNED_INT, 0);
        }

        glUniformMatrix4fv(basic_shader["uT"], 1, GL_FALSE, &carRes[0][0]);
